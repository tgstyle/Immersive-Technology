...
public class GuiLoadController extends GUICommonValve {
	...
	public GuiLoadController(TileEntityLoadController tile) {
		this.tile = tile;
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawString(this.fontRenderer, TranslationKey.GUI_LOAD_CONTROLLER_LIMIT_PACKET.text(), width / 2 - 30, height / 2 - 13, Color.WHITE.getRGB());
		limitTime.drawTextBox();
		drawString(this.fontRenderer, TranslationKey.GUI_LOAD_CONTROLLER_LIMIT_TIME.text(), width / 2 - 30, height / 2 + 5, Color.WHITE.getRGB());
		destinationKeep.drawTextBox();
		drawString(this.fontRenderer, TranslationKey.GUI_LOAD_CONTROLLER_LIMIT_DESTINATION.text(), width / 2 - 30, height / 2 + 23, Color.WHITE.getRGB());
